"""
data_sources/weather.py — WeatherAPI.com + static fallback.
"""
import httpx
...
async def fetch_weather(destination: str, month: int) -> dict:
    """Return weather data for a destination/month as a Python dict."""
    ...
    climate = VIETNAM_CLIMATE.get(dest_key, {}).get(
        month,
        {"condition": "unknown", "temp_c": 27, "rain_chance": 30, "beach_ok": True},
    )
    result = {
        "success": True,
        "source": "static",
        ...
        "month": month,
        **climate,
        "tip": _weather_tip(climate["rain_chance"], climate["temp_c"]),
    }
